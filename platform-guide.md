...
- Superpowers installed and discoverable on this host. Resolve `writing-plans`,
  `subagent-driven-development`, and `requesting-code-review` to their actual
  `SKILL.md` paths. Inspect the installed SDD skill: this workflow requires its
  `scripts/sdd-workspace` and `scripts/review-package`. If absent, stop and report
  the incompatible dependency rather than inventing paths or installing silently.
- Native nested agents, sharing the target checkout: controller → executor →
  implementer/reviewer. This is three agent levels, or two edges below the root.
  Planner, verifier, and repair are direct children of the controller. They should
  not spawn helpers; only the executor needs nesting. Implementers and reviewers
  are leaves. Check exposed depth/capacity limits; do not infer them from a brand.
- Enough capacity for the controller, executor, and one worker concurrently.
...
- Choose the strongest available model for planner/executor/repair and a capable
  mid-tier model for verifier. Resolve actual IDs from the live tool schema or
  ...
  a model parameter. Executor workers follow SDD's tier guidance the same way.
